...
class EbayCarScraper:
    ...
    def __init__(
        ...
        *,
        ...
        # Ensure we always look like a desktop browser so the markup contains
        # the data-testid attributes and server-rendered values we expect.
        ...
        self.session.headers.setdefault("Accept-Language", "de-DE,de;q=0.9,en;q=0.8")
    ...
    def parse_listing_id(self, soup: BeautifulSoup, url: str) -> Optional[str]:
        ...
        if page_text:
            text_match = re.search(r"(?:Anzeigen[- ]ID|ID)[:\s#]*(\d{5,})", page_text, re.IGNORECASE)
            if text_match:
    ...
    def parse_seller(self, soup: BeautifulSoup) -> SellerInfo:
        ...
        if not phone_number:
            ...
            if tel_link:
                href_value = tel_link.get("href")
                if href_value:
                    href_value = href_value.replace("tel:", "").strip()
                text_value = tel_link.get_text(" ", strip=True)
                phone_number = href_value or text_value
        return SellerInfo(
    ...
    def _derive_label_from_testid(self, test_id: str) -> Optional[str]:
        base = re.sub(r"[-_]?value$", "", test_id, flags=re.IGNORECASE)
        base = re.sub(r"[-_]?content$", "", base, flags=re.IGNORECASE)
        if not base:
            return None
    ...
    def _extract_testid_pairs(self, soup: BeautifulSoup) -> Dict[str, str]:
        attributes: Dict[str, str] = {}
        selector = ",".join(
            [
        ...
        value_elements = soup.select(selector)
        for value_element in value_elements:
            data_testid = value_element.get("data-testid")
            if not data_testid:
                continue
            text = self._normalize_value(value_element.get_text(" ", strip=True))
            if not text:
                continue
            base = re.sub(r"[-_]?value$", "", data_testid, flags=re.IGNORECASE)
            label: Optional[str] = None
            for suffix in ("-label", "-title", "-name", "-key", "-heading"):
                label_element = soup.select_one(f"[data-testid='{base}{suffix}']")
                if label_element:
                    label = self._normalize_value(label_element.get_text(" ", strip=True))
                    if label:
                        break
            if not label:
                parent = value_element.find_parent(
                    attrs={"data-testid": re.compile(re.escape(base), re.IGNORECASE)}
                )
                if parent:
                    candidate = self._normalize_value(parent.get_text(" ", strip=True))
                    if candidate and candidate.endswith(text):
                        candidate = candidate[: -len(text)].strip()
                    label = candidate
            if not label:
                label = self._derive_label_from_testid(data_testid)
            if label:
    ...
    def parse_attribute_rows(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        def add_attribute(key: Optional[str], value: Optional[str]) -> None:
            if not key or not value:
                return
            normalized_key = self._normalize_key(key)
            if not normalized_key or normalized_key in normalized_keys:
                return
        ...
        if selector:
            for row in soup.select(selector):
                parts: List[str] = []
                for part in row.stripped_strings:
                    normalized_part = self._normalize_value(part)
                    if normalized_part:
                        parts.append(normalized_part)
                if len(parts) >= 2:
                    add_attribute(parts[0], " ".join(parts[1:]))

        # Fallback: legacy pages render specification tables with definition
        # lists (dt/dd).
        for definition_list in soup.select("dl"):
            terms = definition_list.find_all("dt")
            descriptions = definition_list.find_all("dd")
            if len(terms) != len(descriptions) or not terms:
                continue
            for term, description in zip(terms, descriptions):
                key = self._normalize_value(term.get_text(" ", strip=True))
                value = self._normalize_value(description.get_text(" ", strip=True))
                add_attribute(key, value)

        for key, value in self._extract_testid_pairs(soup).items():
            add_attribute(key, value)

        return attributes

    def apply_meta_fallbacks(self, listing: CarListing, soup: BeautifulSoup) -> None:
        def meta_content(**attrs: str) -> Optional[str]:
        ...
        if not listing.price:
            amount = meta_content(property="product:price:amount") or meta_content(
                itemprop="price"
            )
            currency = meta_content(property="product:price:currency") or meta_content(
                itemprop="priceCurrency"
            )
            if amount:
                listing.price = f"{amount} {currency}" if currency else amount

    def apply_text_fallbacks(self, listing: CarListing, soup: BeautifulSoup) -> None:
        page_text = soup.get_text(" ", strip=True)
        if not page_text:
            return
        if not listing.view_count:
            view_match = re.search(
                r"(?:Aufrufe|Besucher|Gesehen|Views)[:\s]+([\d\.]+)", page_text, re.IGNORECASE
            )
            if view_match:
                listing.view_count = view_match.group(1)
        if not listing.date:
            date_match = re.search(
                r"(?:Erstellungsdatum|Einstelldatum|Aktualisiert am)[:\s]+([0-9.:-]+)",
                page_text,
                re.IGNORECASE,
            )
            if date_match:
                listing.date = date_match.group(1)
    ...
    def parse_listing_details(self, url: str) -> CarListing:
        ...
        self.apply_meta_fallbacks(listing, soup)
        self.apply_text_fallbacks(listing, soup)
        return listing
