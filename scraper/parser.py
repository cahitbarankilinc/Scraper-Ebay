...
def parse_listing(html: str) -> Listing:
    ...
    if not title:
        title = _extract_text_by_attr(root, "id", ["viewad-title", "ad-title", "title"], ("h1", "h2", "div", "span"))
    if not title:
        title = _extract_text_by_attr(root, "class", ["ad-title", "ad-headline", "ad-headline__title", "headline"], ("h1", "h2", "div", "span"))
    if not title:
        h1 = root.find(tag="h1")
        if h1:
    ...
    if not title:
        title = _extract_from_json_ld(data_blocks, "name") or _extract_from_data_blocks(
    ...
    if not price_text:
        ...
        if price_raw:
            if currency_value and currency_value not in str(price_raw):
                price_text = f"{price_raw} {currency_value}"
            else:
    ...
    for values in (
        ...
        description = _extract_text_by_attr(root, "data-testid", values, ("div", "section", "p"))
        if description:
            break
    if not description:
        description = _extract_text_by_attr(root, "id", ["viewad-description-text", "ad-description"], ("div", "section"))
    if not description:
        description = _extract_text_by_attr(root, "class", ["description", "ad-description", "article-description"], ("div", "section", "p"))
    if not description:
        description = _extract_meta_content(html, ["og:description", "description", "twitter:description"])
    if not description:
        description = _extract_from_json_ld(data_blocks, "description") or _extract_from_data_blocks(
